...
/**
 * Description of Oaterminal
 *
 * @author bright
 */
class Oaterminal extends CI_Controller{
    ...
    function issued($receipt_code,$user_code){
        $receipt = $this->security->xss_clean($receipt_code);
        $user_code = $this->security->xss_clean($user_code);
        
        $issue =  $this->Trip->issue($receipt,$user_code);
        
        if($issue){
            echo json_encode(array('result'=>TRUE));
        }
        
        echo json_encode(array('result'=>FALSE,'status'=>''));
    }
}
